...
def XYZ_to_Yxy(XYZ):
    (x,y,_)=XYZ.shape
    Yxy=np.zeros((x,y,3),dtype='double')
    for i in range(x):
        for j in range(y):
            X = float(XYZ[i][j][0])
            Y = float(XYZ[i][j][1])
...
def Yxy_to_XYZ(Yxy):
    (x1,y1,_)=Yxy.shape
    XYZ=np.zeros((x1,y1,3),dtype='double')
    for i in range(x1):
        for j in range(y1):
            Y = float(Yxy[i][j][0])
            x = float(Yxy[i][j][1])
...
def LinearStreaching(Yxy):
    ...
    for i in range(w):
        for j in range(h):
            Yxy[i][j][0] = (float(Yxy[i][j][0])-a)*(B-A)/(b-a)
    return Yxy
...
Yxy = XYZ_to_Yxy(XYZ)
Yxy_new = LinearStreaching(Yxy)
XYZ = Yxy_to_XYZ(Yxy_new)
